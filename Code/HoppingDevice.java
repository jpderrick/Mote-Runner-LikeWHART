...
public class HoppingDevice {
	...
	private static byte NEW_CHANNEL = BCAST_CHANNEL;
	private static byte FUTURE_CHANNEL = 1;
	private static byte NEW_CHANNEL_TO_SET = 3;
	...
	private static int[] SLOT_FUNCTION = {1,2,1,2,1,2,1,2,2}; //1 = Transmit, 2= Recieve
	private static byte[] DESTINATIONS = {0x11,0x11,0x11,0x11,0x11};
	
	//private static int[] F1_SLOT_CHANNEL = {};
	private static int OFFSET_INDEX = -1; //Set it to -1 so we can keep track
	private static byte[] msg = new byte[(12+MAX_PAYLOAD)];
	...
	static {
			...
			radio.setRxHandler(new DevCallback(null){
				public int invoke (int flags, byte[] data, int len, int info, long time){
					return HoppingDevice.handleFrame(flags, data, len, info, time);
				}
			});
			...
			tslot.setCallback(new TimerEvent(null){
				public void invoke(byte param, long time){
					HoppingDevice.timeSlot(param, time);
				}
			});
	...
	private static int handleFrame(int flags, byte[] data, int len, int info, long time){
			...
			if(data == null){
				...
				return 0;
			}else if(data[11] <= 16) { //IF this is a broadcast i.e. is it defining a superframe??
				...
				Logger.appendString(csr.s2b("Sync Recieved"));
 				Logger.flush(Mote.WARN);
				tslot.setAlarmBySpan(Time.toTickSpan(Time.MILLISECS, SLOT_TABLE[OFFSET_INDEX] * TIME_SLOT_LENGTH));
			}else{
				...
				//This is where we write to the logge
	...
	public static void timeSlot(byte param, long time){
		...
		if(OFFSET_INDEX == SLOT_TABLE.length){
			...
			OFFSET_INDEX++;
			if(OFFSET_INDEX == SLOT_TABLE.length){
				//No more operations to do, so change the channel for the next SF
				radio.stopRx();
				radio.setChannel(FUTURE_CHANNEL);
